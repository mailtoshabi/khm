                                        ...
                                        <div class="{{ (Utility::get_stock($cartitem->id,$cartitem->attributes->type) == 0) || (Utility::get_stock($cartitem->id,$cartitem->attributes->type) < $cartitem->quantity) ? 'bg-danger' : '' }}" id="prod_container_{{ $index }}" style="padding: 10px;">
                                            <div class="row">
                                                ...
                                                <div class="col-xs-6 col-md-4">
                                                    ...
                                                    @endif
                                                    <p class="product-name"><a target="_blank" href="{{ route('all.slug',Utility::getProductSlug($cartitem->id)) }}"> {{ $cartitem->name }}</a><br>
                                                ...
                                                <div class="col-xs-12 col-md-6">
                                                    <div class="col-xs-5 text-right">
                                                        <h6><strong><i class="fa fa-inr"></i><span id="cartItemPrice_{{ $index }}">{{ $cartitem->price }}</span> <span class="text-muted">x</span></strong></h6>
                                                    </div>
                                                    <div class="col-xs-3">
                                                        <input id="prod_quanity_{{ $index }}" type="text" class="form-control input-sm" value="{{ $cartitem->quantity }}">
                                                    </div>
                                                    <div class="col-xs-2">
                                                        <button type="button" class="btn btn-link btn-xs update_cart" data-index="{{ $index }}" data-action="{{ route('product.update_cart',[$index]) }}">
                                                            <span class="glyphicon glyphicon-refresh text-primary"> </span>
                                                        </button>
                                                    </div>
                                                    <div class="col-xs-2">
                                                        <button type="button" class="btn btn-link btn-xs delete_cart" data-index="{{ $index }}" data-action="{{ route('product.delete_cart',[$index]) }}">
                                ...
                                <div class="panel-footer">
                                    <div class="row">
                                        <div class="col-xs-12 col-md-9">
                                            <button class="back-to-product btn btn-default" type="button" onclick="location.href='{{ route('category.all') }}'"><span class="fa fa-angle-left"></span> &nbsp;&nbsp;CONTINUE SHOPPING</button>
                                        </div>
                                        <div class="col-xs-12 col-md-3">
                                            <button type="button" class="place_order btn btn-default ladda-button" data-style="zoom-out" style="color: #fff; font-weight: bold" onclick="location.href='{{ Auth::guard('customer')->guest() ? route('checkout.login') : route('checkout.address') }}'" >
...
<script>
    $(document).ready(function() {
        ...
        $("#empty_cart").click(function(e){
            e.preventDefault();
            var url = "{{ route('product.cart.clear') }}";
        ...
        $(".update_cart").click(function(e){
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    priceContainer.text(data.price);
