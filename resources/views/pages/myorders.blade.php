                                        ...
                                        <div id="salerow" class="row">
                                                ...
                                                <h4 class="product-name">
                                                    ...
                                                    @if(($sale->pay_method == Utility::PAYMENT_OFFLINE) && (!$sale->is_paid == 1))
                                                        <label class="utr_no_div">
                                                            ...
                                                            <p><small>Make the payment by transferring the amount to <a onclick="event.preventDefault()" id="bank_details_{{ $sale->id }}" href="#">Kerala health Mart Bank account</a> and enter your payment referance number here.</small></p>
                                                    @endif
                                            ...
                                            <div class="col-lg-6 col-md-6 col-sm-6 col-xs-6">
                                                ...
                                                <div id="order_cancel" class="col-lg-4 col-md-4 col-sm-4 col-xs-3" style="padding-top: 7px;">
                                                    @if($sale->is_paid)
                                                        <p style="font-weight: bold; font-size:20px;"><a target="_blank" href="{{ route('bill.download.specific',[$sale->id,'download'=>'invoice']) }}" ><small class="label bg-green fa fa-download"> Download Bill</small></a></p>
                                                    @else
                                                        <p style="font-weight: bold; font-size:20px;"><a href="#" class="pay_bill" data-url="{{ route('checkout.pay.later',encrypt($sale->id)) }}"><small class="label bg-green fa fa-inr"> Pay Bill</small></a></p>
                                                    @endif
                                                    @if($sale->status == Utility::SALE_STATUS_NEW)
                                                            <p style="font-weight: bold; font-size:20px;"><a href="{{ route('order.cancel') }}" data-plugin="ajaxGetRequest" data-type="GET" data-formdata='[{"sale":"{{ $sale->id }}"}]' data-conf-message="Do you really want to cancel the order..?"><small class="label bg-red fa fa-remove"> Cancel Order</small></a></p>
                                                    @endif
                                                </div>
                                            </div>
                                        </div>
