...
<nav class="navbar navbar-expand-lg fixed-top text-uppercase" id="mainNav" style="background: #8AD5FD;">
    <div class="container">
        <a class="navbar-brand js-scroll-trigger" href="#gmap" style="line-height: 20px; text-align: center; padding-left: 10px;">
            {{ $clinic->user->name }}
            <small id="location_text" style="font-size: 15px;"><br><i style="font-size: 16px;" class="fa fa-map-marker"></i> <b>{{ $clinic->get_city() }}</b>, {{ $clinic->get_district() }} {{ !empty($clinic->pin)? ' - ' . $clinic->pin :'' }}</small>
        </a>
                ...
                <div class="row">
                    <div class="col-md-12">
                        <div class="row clinic_images portfolio">
                            ...
                            <div id="clinic-slider" class="content-slider">
...
{{--<section id="treatment_offered" class="bg-blackblue text-white mb-0">
    <div class="container">
        <div class="row">
            <div class="col-lg-12 ml-auto">
                --}}{{--<h3 class="text-center text-uppercase text-white">Treatments Offered</h3>--}}{{--
                @if(!empty(json_decode($clinic->treatments)))
                    <?php $treatment_no = 1; ?>
                    @foreach($clinic->treatments as $clinic_treatment)
                        <div @if(count($clinic->treatments) != $treatment_no) style="padding-bottom: 25px;" @endif>
                            <h5>{{ $clinic_treatment->name }}</h5>
                            {!! str_limit($clinic_treatment->description, 250, '...') !!}
                            @if(strlen(str_limit($clinic_treatment->description, 250, '')) != strlen($clinic_treatment->description))
                                <a href="#" data-toggle="modal" data-target="#treatmentModal_{{ $treatment_no }}"><i class="fa fa-angle-double-right readmore"></i><i class="fa fa-angle-double-right readmore"></i></a>
                            @endif
                            @if(!empty($clinic_treatment->images))
                                <div id="treatment_slider_{{ $treatment_no }}" class="content-slider">
                                    @foreach($clinic_treatment->images as $treatment_image)
                                        <img class="img-fluid" src="{{ asset(Utility::DEFAULT_STORAGE . $treatment_image['original']) }}" alt="">
                                    @endforeach
                                </div>
                            @endif
                        </div>
                        @if(count($clinic->treatments) != $treatment_no)
                            <hr>
                        @endif
                    <?php $treatment_no++; ?>
                    @endforeach
        ...
        $.ajax({
            ...
            success: function (data) {
                $("#jq-loader").fadeOut(50, function() {
                    $('#dv_doctors').empty().append(data.content);
