                ...
                <div class="col-sm-12">
                    <div>
                        <center>
                            ...
                            <i class="fa fa-phone" style="color:#666"></i> {{ $sale->customer->phone }} {{ (!empty($sale->customer->customer_detail->address) && ($sale->customer->customer_detail->address['phone_alt']!='') ) ? ', ' . $sale->customer->customer_detail->address['phone_alt'] : '' }} <br>
                            {{--{!! $sale->customer->customer_detail->address['address'] . '<br>' !!}--}}
                            {{--{{ $sale->customer->customer_detail->address['place'].', ' }} {{ $sale->customer->customer_detail->address['city'].', ' }} {{ $sale->customer->customer_detail->address['pincode'].', ' }}<br>--}}
                            {{--{{ Utility::district_name($sale->customer->customer_detail->address['district']) .', ' }} {{ Utility::state_name($sale->customer->customer_detail->address['state']) .', '}} India.--}}
                    ...
                    <div class="">
                        <div class="col-xs-10">
                            <address>
                                ...
                                <br>
                                {{--<strong>{!! $sale->pay_method == Utility::PAYMENT_COD ? '<small class="label label-primary">Cash on Delivery</small>' : '<small class="label label-primary">Online Payment</small>' !!} </strong>--}}
        ...
        $(document).ready(function(){
            ...
            $('#dv_pay_link').on('ajax-get-request.success', function (e,data) {
                /*if(data.is_paid == 1) {
                    ...
                    var payment_form_data = '[{"payment":"{{ Utility::SALE_NOTPAID }}","sale":"{{ $sale->id }}"}]';
                    var obj = JSON.parse(payment_form_data);
                    var payment_confirm =  'Do you really want to Cancel the Payment?';
                    var payment_confirm_data =  '{{ Utility::SALE_NOTPAID }}';
                    var payment_display =  'Cancel Payment';
                    $('#pay_status').text('Paid').addClass('btn-success').removeClass('btn-danger');
                }
                else {
                    payment_class =  'text-success';
                    payment_form_data = '[{"payment":"{{ Utility::SALE_PAID }}","sale":"{{ $sale->id }}"}]';
                    obj = JSON.parse(payment_form_data);
                    payment_confirm =  'Do you really want to Mark as Paid?';
                    payment_confirm_data =  '{{ Utility::SALE_PAID }}';
                    payment_display =  'Mark as Paid';
                    $('#pay_status').text('Not Paid').addClass('btn-danger').removeClass('btn-success');
                }
                $('#pay_link').attr('class',payment_class).attr('data-formdata',obj).attr('data-conf-message',payment_confirm).attr('data-confdata',payment_confirm_data).text(payment_display);*/
                location.reload();
            });
                        ...
                        success: function (data) {
                            if (data.success) {
                                /*if(data.paid_status==1) {
                                    $('#pay_status').removeClass('btn-danger').addClass('btn-success').text('Paid');
                                }else {
                                    $('#pay_status').removeClass('btn-success').addClass('btn-danger').text('Not Paid');
                                }*/
                                location.reload();
