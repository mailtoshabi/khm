...
<section class="content">
    <div class="row">
        <!-- form start -->
        <form action="@if(isset($product->id)) {{ route('admin.brands.products.update',[$product->id]) }} @else {{ route('admin.brands.products.store') }}  @endif" method="POST" role="form" id="add-product-form" enctype="multipart/form-data">
            {!! csrf_field() !!}
            <div class="col-md-8">
...
<script>
    $(document).ready(function() {
        ...
        @if(isset($product->id))
            ...
            var url = '{{ route('admin.brands.products.sidebar_categories',[$product->id]) }}';

        @else

            url = '{{ route('admin.brands.products.sidebar_categories') }}';
